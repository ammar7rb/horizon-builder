...
const ContactEditor = () => {
  ...
  const updateField = (field: string, value: string) => {
    updateContent((prev) => ({ ...prev, contact: { ...prev.contact, [field]: value } }));
  };
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <input value={contact.tagline} onChange={(e) => updateField("tagline", e.target.value)} className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="font-body text-xs text-muted-foreground">Title</label>
          <input value={contact.title} onChange={(e) => updateField("title", e.target.value)} className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="font-body text-xs text-muted-foreground">Description</label>
          <input value={contact.description} onChange={(e) => updateField("description", e.target.value)} className={inputClass} />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <input value={contact.email} onChange={(e) => updateField("email", e.target.value)} className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="font-body text-xs text-muted-foreground">Office</label>
          <input value={contact.office} onChange={(e) => updateField("office", e.target.value)} className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="font-body text-xs text-muted-foreground">Availability</label>
          <input value={contact.availability} onChange={(e) => updateField("availability", e.target.value)} className={inputClass} />
        </div>
      </div>
    </div>
  );
