...
const GeneralSettings = () => {
  ...
  const handleLogoType = useCallback(
    (type: "text" | "image") => {
      updateContent((prev) => ({ ...prev, config: { ...prev.config, logoType: type } }));
    },
    [updateContent]
  );

  const handleFileUpload = useCallback(
    (field: "logoImage" | "favicon", file: File) => {
      const reader = new FileReader();
      reader.onload = (e) => {
        const result = e.target?.result as string;
        updateContent((prev) => ({ ...prev, config: { ...prev.config, [field]: result } }));
      };
    ...
    <div className="space-y-8">
      ...
      {config.logoType === "text" && (
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) => updateContent((prev) => ({ ...prev, config: { ...prev.config, logoText: e.target.value } }))}
      ...
      {config.logoType === "image" && (
        <div className="space-y-2">
          ...
          />
          {config.logoImage && (
            <img src={config.logoImage} alt="Logo preview" className="h-10 mt-2 object-contain" />
          )}
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        />
        {config.favicon && (
          <img src={config.favicon} alt="Favicon preview" className="h-8 mt-2" />
        )}
      </div>
